<form action="main.php" method="POST">
    Date <input type = "date" name = "sana"><br>
    Arrived At <input type="TIME" name="kelgan_vaqt"><br>
    Leaved At <input type="TIME" name="ketgan_vaqt"><br>
...
if(!empty($_POST)){
    if ($_POST['kelgan_vaqt'] != ""  &&  $_POST['ketgan_vaqt'] && $_POST['sana'] != ""){
        ...
        $vaqt = new main($kelvaqt,$ketvaqt);
        $ishlagansoati = $vaqt -> Ishlagan_soati();
        $qarzvaqti = $vaqt -> Qarz_soati();

        $pdo = new PDO(
            $dsn = 'mysql:host=localhost;dbname=work_of_trekker',
            $username = 'root',
            $password = '3702');
        ...
        echo "Ma'lumot kiritmadingiz";
    }

    $query = $pdo-> query("select * from ish_soati")->fetchAll();
    $umumiyMINUT = NULL;
    foreach($query as $row){
        $time1 = explode(":", $row['qarz_vaqti']);
        $minutlar = ($time1[0] * 60) + $time1[1];
